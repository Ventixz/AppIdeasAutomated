/* script.js — thin UI layer over e-core.js. All maths lives in the core. */

'use strict';

(function () {
  var core = window.ECore;
  ...
  function setMode(next) {
    ...
    if (expand) {
      ...
      label.textContent = 'Which digit? (1 = the “2”, 2 = first decimal, …)';
      input.min = '1';
      if (Number(input.value) < 1) input.value = '1';
      hint.textContent = '1-indexed. 1 gives 2, 2 gives 7, 3 gives 1, …';
    }
  ...
  function renderExpand(n) {
    ...
    var s = core.eString(n); // "2" or "2.xxxx"
    var ms = Math.round(performance.now() - t0);
    resultLabel.textContent = 'e to ' + n + (n === 1 ? ' digit' : ' digits') + ' after the decimal';
    if (n === 0) {
      out.innerHTML = '<span class="int">2</span>';
    } else {
      out.innerHTML =
        '<span class="int">2</span><span class="dot">.</span>' + s.slice(2);
    }
  ...
  function renderSingle(n) {
    ...
    resultLabel.textContent = 'Digit #' + n + ' of e (' + place + ')';
